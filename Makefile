...
export CFLAGS= -DDMLC_LOG_CUSTOMIZE=1 -std=c++11 -Wall -static-libstdc++ -Wno-unknown-pragmas -Iinclude $(ADD_CFLAGS) $(PLUGIN_CFLAGS)
...
lib/xgboost.dll lib/libxgboost.so lib/libxgboost.dylib: $(ALL_DEP)
	@mkdir -p $(@D)
	$(CXX) $(CFLAGS) -shared -static-libstdc++ -o $@ $(filter %.o %a,  $^) $(LDFLAGS)
